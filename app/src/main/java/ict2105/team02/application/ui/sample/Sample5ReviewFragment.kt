...
class Sample5ReviewFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.buttonSendSample.setOnClickListener {
            val confirmationDialog = ConfirmationDialogFragment(resources.getString(R.string.confirm_sample_prompt)) {
                // User clicked confirm
                sampleViewModel.insertSampleData()
                Toast.makeText(
                    requireContext(),
                    resources.getString(R.string.confirm_sample_success),
                    Toast.LENGTH_LONG
                ).show()
